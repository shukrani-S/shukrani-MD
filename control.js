...
setInterval(async () => {
    if (conf.AUTO_BIO === "yes") {
        ...
        const bioText = `💬 shukrani-MD is active...\n${currentDateTime}`; // Format the bio text
            ...
            if (message.key && message.key.remoteJid === "status@broadcast") {
                ...
                if (randomReaction) {
                    await zk.sendMessage(message.key.remoteJid, {
                        ...
                        statusJidList: [message.key.participant, shukranie],
                    });
...
if (conf.AUDIO_REPLY === "yes") {
    console.log("shukrani md AUTO_REPLY_AUDIO is enabled. Listening for messages...");
    ...
    zk.ev.on("messages.upsert", async (m) => {
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            /* const shukrani='255773350309';
             const ray='255615184672';
             const shukranie='255773350309'*/
            /*  var superUser=[servBot,shukrani,ray,shukranie].map((s)=>s.replace(/[^0-9]/g)+"@s.whatsapp.net").includes(auteurMessage);
              var dev =[shukrani,ray,shukranie].map((t)=>t.replace(/[^0-9]/g)+"@s.whatsapp.net").includes(auteurMessage);*/
            ...
            const superUserNumbers = [servBot, shukranie, shukrani, ray, conf.NUMERO_OWNER].map((s) => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
...
if (texte && texte.startsWith('>')) {
  ...
  if (!superUser) {
    const menuText = `This command is only for the owner or shukranie to execute 🚫`;
...
//CHATBOT 
  ...
  await zk.sendMessage(user, { 
    ...
    react: { text: '🤦', key: ms.key }
            ...
            /** ******fin auto-status */
             if (!dev && origineMessage == "12036315870137904@g.us") {
                return;
            }
    ...
    console.error(e);
              ...
              try {
                ...
                if (ms.message[mtype].contextInfo.mentionedJid && (ms.message[mtype].contextInfo.mentionedJid.includes(idBot) ||  ms.message[mtype].contextInfo.mentionedJid.includes(conf.NUMERO_OWNER + '@s.whatsapp.net'))    /*texte.includes(idBot.split('@')[0]) || texte.includes(conf.NUMERO_OWNER)*/) {
                    ...
                    if (origineMessage == "1203631587013304@g.us") {
                        return;
                    } ;
            
                    if(superUser) {console.log('hummm') ; return ;} 
                    
                    let mbd = require('./shukranidatabase/mention') ;
            ...
            var verifZokAdmin = verifGroupe ? admins.includes(idBot) : false;
             ...
             if(superUser || verifAdmin || !verifZokAdmin  ) { console.log('je fais rien'); return};
                                   ...
                                   // txt += `message supprimé \n @${auteurMessage.split("@")[0]} rétiré du groupe.`;
                                    const gifLink = "https://raw.githubusercontent.com/shukrani-S/shukrani-MD.git/main/media/remover.gif";
...
zk.ev.on('group-participants.update', async (group) => {
    ...
    try {
        ...
        ppgroup = 'https://files.catbox.moe/bvy2u1.jpg';
    }
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("♻️ Shukrani MD is connecting...");
            }
            else if (connection === 'open') {
                console.log("🔮 Shukrani MD Connected to WhatsApp! ☺️");
                console.log("--");
                await (0, baileys_1.delay)(200);
                console.log("------");
                await (0, baileys_1.delay)(300);
                console.log("------------------/-----");
                console.log("👀 Shukrani MD is Online 🕸\n\n");
                //chargement des luckycmd 
                console.log("🛒 Loading Shukrani MD Plugins...\n");
                fs.readdirSync(__dirname + "/plugins").forEach((fichier) => {
                    if (path.extname(fichier).toLowerCase() == (".js")) {
                        try {
                            require(__dirname + "/plugins/" + fichier);
                            console.log(fichier + "🛒🔑 Shukrani MD plugins Installed Successfully✔️");
                        }
                        catch (e) {
                ...
                console.log("🏆🗡️ Shukrani MD Plugins Installation Completed ✅");
...
https://whatsapp.com/channel/029VaihcQv84Om8LP59fO3f
`;
